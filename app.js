...
$(document).ready(function () {
    ...
    // Here is the difficult time bit I TRIED AND I TRIED.

    function updateMinutes() {
        var present = moment();
        var freqminutes = snapshot.val().Recurrency;
        var StartTime = moment(snapshot.val().StartTime, "HH:mm").subtract(1 - "years");
        var timeDiff = present.diff(moment(StartTime), "minutes");
        var remanenttime = timeDiff % freqminutes;
        var minutesremaining = freqminutes - remanenttime;
        var nexttime = moment().add(minutesremaining, "minutes");

        StartTime.text(moment(nexttime).format("HH:mm"));
        NextTrain.text(minutesremaining);
    }
    updateMinutes();
